"""add shared husbandry queue and review invariants
...
def upgrade() -> None:
    # This migration mirrors the user-service migration so either deployment
    # order establishes the cross-service database invariants.
    op.execute("""
        WITH ranked AS (
          SELECT id,
                 row_number() OVER (
        ...
        UPDATE admin.work_items item
           SET status = 'cancelled', completed_at = COALESCE(item.completed_at, now()),
    ...
    """)
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_work_items_active_resource ON admin.work_items (resource_type, resource_id) WHERE status IN ('open', 'claimed')")
    op.execute("""
        WITH ranked AS (
          SELECT id,
                 row_number() OVER (PARTITION BY husbandry_case_id ORDER BY version ASC, created_at ASC, id ASC) AS next_version
    ...
    """)
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_expert_reviews_husbandry_case_version ON admin.expert_reviews (husbandry_case_id, version) WHERE husbandry_case_id IS NOT NULL")
    op.execute("""
        ...
        BEGIN
          IF NEW.status <> 'closed' AND NEW.severity IN ('high', 'critical') THEN
            IF to_regclass('admin.work_items') IS NOT NULL THEN
              ...
              SELECT 'high_risk_case', 'husbandry_case', NEW.id::text,
                     '复核高风险养殖病例：' || NEW.title,
    ...
    op.execute("CREATE TRIGGER trg_husbandry_cases_enqueue_high_risk_review AFTER INSERT OR UPDATE OF severity, status ON husbandry_cases FOR EACH ROW EXECUTE FUNCTION public.enqueue_husbandry_high_risk_review()")
