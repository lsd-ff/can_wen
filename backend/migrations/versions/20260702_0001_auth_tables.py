...
def upgrade() -> None:
    ...
    op.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS uq_user_identities_active_subject
    ...
    op.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS uq_user_identities_primary_per_provider
    ...
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_auth_verification_codes_target
    ...
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_login_events_target_created_at
...
def downgrade() -> None:
    op.execute("DROP TRIGGER IF EXISTS trg_auth_sessions_updated_at ON auth_sessions")
    op.execute("DROP TRIGGER IF EXISTS trg_auth_verification_codes_updated_at ON auth_verification_codes")
    op.execute("DROP TRIGGER IF EXISTS trg_user_identities_updated_at ON user_identities")
    op.execute("DROP TRIGGER IF EXISTS trg_users_updated_at ON users")
    op.execute("DROP TABLE IF EXISTS login_events")
    op.execute("DROP TABLE IF EXISTS auth_sessions")
    op.execute("DROP TABLE IF EXISTS auth_verification_codes")
...
def _recreate_updated_at_triggers() -> None:
    triggers = {
        ...
        "trg_user_identities_updated_at": "user_identities",
        "trg_auth_verification_codes_updated_at": "auth_verification_codes",
        "trg_auth_sessions_updated_at": "auth_sessions",
    }
